...
import {
  ...
  isSuccessfulErpOrderSync,
  normalizeErpParameterCode,
...
const toErpOptions = (payload: unknown): ErpOption[] => {
  ...
  return rows
    .map((row: unknown) => {
      ...
      const rawCode = readFirstText(record, [
        "code",
        "codigo",
        "CODIGO",
        "value",
      ...
      const name =
        readFirstText(record, ["name", "nome", "NOME"]) || description || code;
      const label =
        readFirstText(record, ["label", "LABEL"]) ||
...
const findDefaultSalesOperationCode = (options: ErpOption[]) =>
...
function StatusPill({
  ...
  tone: keyof typeof statusPillClassName;
  children: string;
}) {
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${statusPillClassName[tone]}`}
    >
      {children}
    </span>
  );
...
function SearchableSelect({
  ...
  return (
    <label className="relative block text-sm">
      ...
      />
      {open && !loading ? (
        <div className="absolute z-50 mt-1 max-h-56 w-full overflow-auto rounded-xl border border-slate-200 bg-white p-1 shadow-xl shadow-slate-900/10">
          {filteredOptions.length ? (
            filteredOptions.map((option) => (
              <button
                ...
                className={`w-full rounded-lg px-2.5 py-1.5 text-left text-xs transition hover:bg-brand-50 ${option.value === value ? "bg-brand-50 text-brand-800" : "text-slate-700"}`}
              >
...
export default function OpportunityDetailsPage() {
  ...
  const [clientErpSummary, setClientErpSummary] =
    useState<ClientErpSummary | null>(null);
  const [opportunityItems, setOpportunityItems] = useState<OpportunityItem[]>(
    [],
  );
  const [opportunityItemTotals, setOpportunityItemTotals] =
    useState<OpportunityItemTotals>({
      grossTotal: 0,
      discountTotal: 0,
      netTotal: 0,
    });
  ...
  const [erpOrderForm, setErpOrderForm] =
    useState<ErpOrderForm>(emptyErpOrderForm);
  const [erpOrderFeedback, setErpOrderFeedback] =
    useState<ErpOrderFeedback | null>(null);
  ...
  const load = async () => {
    ...
    try {
      ...
      setOpportunityItemTotals(
        itemsResponse.data?.totals || {
          grossTotal: 0,
          discountTotal: 0,
          netTotal: 0,
        },
      ...
      setErpOrders(
        Array.isArray(erpOrdersResponse.data?.items)
          ? erpOrdersResponse.data.items
          : [],
      );
    } catch {
  ...
  const clientErpCode =
    clientErpSummary?.code?.trim() || item?.clientData?.code?.trim() || "";
  ...
  const setErpOrderField = (
    field: keyof Omit<ErpOrderForm, "simulateOnly">,
    value: string,
  ) => {
  ...
  const loadErpOrderData = async () => {
    ...
    try {
      ...
      const nextReceivingConditions = toErpOptions(
        receivingConditionsResponse.data,
      );
      ...
      setErpOperationDefaultAlert(
        defaultSalesOperationCode ? "" : missingDefaultSalesOperationMessage,
      );
      if (!defaultSalesOperationCode)
        toast.warning(missingDefaultSalesOperationMessage);
      setErpOrderForm((current) => ({
        ...current,
        paymentMethodCode:
          current.paymentMethodCode || firstOptionCode(nextPaymentMethods),
        receivingConditionCode:
          current.receivingConditionCode ||
          firstOptionCode(nextReceivingConditions),
        priceTableCode:
          current.priceTableCode ||
          item.priceTableCode ||
          firstOptionCode(nextPriceTables) ||
          "1",
        branchCode: current.branchCode || firstOptionCode(nextBranches),
        operationCode: defaultSalesOperationCode,
        expectedDeliveryDate:
          current.expectedDeliveryDate || getTodayDateInputValue(),
      }));
    } catch (error) {
      toast.error(
        getApiErrorMessage(error, "Não foi possível carregar dados do ERP"),
      );
    } finally {
  ...
  const openErpOrderModal = () => {
    ...
    loadErpOrderData().catch((error) =>
      toast.error(
        getApiErrorMessage(error, "Não foi possível preparar o pedido ERP"),
      ),
    );
  };
  ...
  const onSendErpOrder = async () => {
    ...
    try {
      ...
      setErpOrderFeedback({
        status: response.data?.simulated ? "simulado" : "enviado",
        pedidoIdImportacao: response.data?.pedidoIdImportacao,
        erpOrderNumber: response.data?.erpOrderNumber,
      ...
      toast.success(
        response.data?.simulated
          ? "Simulação ERP validada sem envio real"
      ...
      const errorCorrelationId =
        maybeResponse?.correlationId ||
        maybeHeaders["x-correlation-id"] ||
        maybeHeaders["X-Correlation-Id"];
      const baseMessage = getApiErrorMessage(
        error,
        "Erro ERP ao enviar pedido",
      );
      const message =
        errorCorrelationId && !baseMessage.includes(String(errorCorrelationId))
          ? `${baseMessage} (correlationId: ${errorCorrelationId})`
          : baseMessage;
      setErpOrderFeedback({
        status: "erro",
  ...
  const onSyncErpOrderStatus = async () => {
    ...
    try {
      const response = await api.post(
        `/opportunities/${item.id}/erp/orders/status`,
      );
      setErpOrders(
      ...
      toast.error(
        getApiErrorMessage(error, "Não foi possível atualizar status ERP"),
      );
    } finally {
  ...
  const onDownloadErpOrderPdf = async (order: ErpOrderSync) => {
    ...
    try {
      const response = await api.get(
        `/opportunities/${item.id}/erp/orders/${order.id}/pdf`,
        { responseType: "blob" },
      );
      ...
      let message = getApiErrorMessage(
        error,
        "Não foi possível gerar o PDF do pedido ERP. Verifique sua sessão e tente novamente.",
      );
  ...
  const onAdvanceStage = async () => {
    ...
    const nextStage =
      stageFlow[Math.min(currentIndex + 1, stageFlow.length - 1)];
  ...
  const onMarkLost = async (event: FormEvent) => {
    ...
    await registerEvent({
      type: "status",
      description: `Motivo da perda: ${lossReason.trim()}`,
    });
  ...
  const onGenerateSalesMessage = async () => {
    ...
    try {
      const response = await api.get("/ai/opportunity-message", {
    ...
    <div className="space-y-4 pb-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <div className="mobile-action-stack md:justify-end">
          <button
            ...
            title={
              !canOpenErpOrder
                ? erpOrderDisabledReason || undefined
                : successfulErpOrder
                  ? "Pedido ERP já enviado; abra para consultar o histórico."
                  : undefined
            }
          >
            {successfulErpOrder
      ...
      {!canOpenErpOrder ? (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
          <strong>Gerar pedido ERP indisponível:</strong>{" "}
          {erpOrderDisabledReason?.replace("Pedido ERP indisponível: ", "")}
        </div>
      ) : null}
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
        ...
        <div className="grid gap-2 text-sm md:grid-cols-2 lg:grid-cols-3">
          ...
          <p>
            ...
            {formatPercentBR(item.probability || 0, 0)}
          </p>
          <p>
            <strong>Ponderado:</strong>{" "}
            {formatCurrencyBRL(item.weightedValue || 0)}
          </p>
          <p>
            <strong>Entrada proposta:</strong> {formatDateBR(item.proposalDate)}
          </p>
          <p>
          ...
          <p>
            <strong>Previsão plantio:</strong>{" "}
            {formatDateBR(item.plantingForecastDate || "")}
          </p>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
        ...
        <div className="grid gap-2 text-sm md:grid-cols-2 lg:grid-cols-3">
          <p>
            <strong>Cultura:</strong> {item.crop || "-"}
          </p>
          <p>
            <strong>Safra:</strong> {item.season || "-"}
          </p>
          <p>
            <strong>Área (ha):</strong> {item.areaHa ?? "-"}
          </p>
          <p>
            <strong>Produto ofertado:</strong> {item.productOffered || "-"}
          </p>
          <p>
            <strong>Ticket por ha:</strong>{" "}
            {item.expectedTicketPerHa
              ? formatCurrencyBRL(item.expectedTicketPerHa)
              : "-"}
          </p>
          <p>
            <strong>Valor/ha:</strong>{" "}
            {valuePerHa ? formatCurrencyBRL(valuePerHa) : "-"}
          </p>
          <p>
            <strong>Ticket estimado total:</strong>{" "}
            {estimatedTicketTotal
              ? formatCurrencyBRL(estimatedTicketTotal)
              : "-"}
          </p>
        </div>
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
        ...
        {insight ? (
          <div className="space-y-2 text-sm">
            <p>
              <strong>Risco:</strong>{" "}
              <span className={riskClassName[insight.risk]}>
                {riskLabel[insight.risk]}
              </span>
            </p>
            <p>
              <strong>Próxima ação:</strong> {insight.nextAction}
            </p>
            <p>
              <strong>Mensagem:</strong> {insight.message}
            </p>
            <div className="mt-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
              <p>
                <strong>Intenção detectada:</strong>{" "}
                {insight.observationInsight?.detectedIntent || "indefinido"}
              </p>
              <p>
                <strong>Interesse:</strong>{" "}
                {insight.observationInsight?.interestLevel || "medio"}
              </p>
              <p>
                <strong>Palavras-chave:</strong>{" "}
                {insight.observationInsight?.keywords?.length
                  ? insight.observationInsight.keywords.join(", ")
                  : "-"}
              </p>
            </div>
          </div>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
        ...
        {salesMessage ? (
          ...
          <p className="text-sm text-slate-500">
            Gere um texto rápido para contato com o cliente.
          </p>
        )}
      </section>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
        ...
        <p className="text-sm">
          ...
          {item.lastContactAt
            ? formatDateBR(item.lastContactAt)
        ...
        <form className="mt-3 space-y-2" onSubmit={onRegisterInteraction}>
          <textarea
            ...
            rows={3}
            placeholder="Escreva um resumo da interação"
            ...
            events.map((event) => (
              ...
              >
                ...
                <p className="mt-1 text-xs text-slate-500">
                  por {event.ownerSeller?.name || "Usuário"}
                </p>
              </div>
            ))
          ) : (
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
        ...
        <div className="mobile-action-stack">
          <button
            ...
            className="mobile-secondary-half rounded-lg border border-slate-300 px-3 py-2 text-sm disabled:opacity-50"
          >
            Avançar etapa
          </button>
          <button
            ...
            disabled={saving}
            onClick={onMarkWon}
            className="mobile-primary-button rounded-lg bg-emerald-600 px-3 py-2 text-sm text-white disabled:opacity-50"
          >
            Marcar como ganho
          </button>
          <button
            ...
            className="mobile-primary-button rounded-lg bg-red-600 px-3 py-2 text-sm text-white disabled:opacity-50"
          >
            Marcar como perdido
          </button>
        </div>
                      ...
                      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <h4 className="mt-1 break-words text-base font-bold text-slate-900">
                              {clientErpSummary?.name ||
                                item.client ||
                                clientErpSummary?.fantasyName ||
                                "Cliente"}
                            </h4>
                            <p className="text-xs text-slate-500">
                              {clientErpSummary?.fantasyName
                                ? `Fantasia: ${clientErpSummary.fantasyName} · `
                                : ""}
                              {[clientErpSummary?.city, clientErpSummary?.state]
                                .filter(Boolean)
                        ...
                        <div className="mt-3 grid gap-2 text-xs sm:grid-cols-2">
                          <div className="rounded-xl bg-slate-50 p-2.5">
                            ...
                            <strong>{clientErpCode || "Sem vínculo"}</strong>
                          </div>
                          <div className="rounded-xl bg-slate-50 p-2.5">
                            <span className="block text-xs text-slate-500">
                              CPF/CNPJ
                            </span>
                            <strong>{clientErpSummary?.cnpj || "-"}</strong>
                          </div>
                        </div>
                      </section>
                      ...
                      <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="text-sm text-slate-500">
                              Responsável comercial da oportunidade
                            </p>
                          </div>
                          {sellerErpCode ? (
                            <StatusPill tone="success">
                        ...
                        <div className="mt-3 grid gap-2 text-xs sm:grid-cols-2">
                          <div className="rounded-xl bg-slate-50 p-2.5">
                            ...
                            <strong>{sellerErpCode || "Sem vínculo"}</strong>
                          </div>
                          <div className="rounded-xl bg-slate-50 p-2.5">
                            <span className="block text-xs text-slate-500">
                              Operador
                            ...
                            <strong>
                              {item.ownerSeller?.erpOperatorCode || "-"}
                            </strong>
                          </div>
                        </div>
                      </section>
                    ...
                    <section className="rounded-2xl border border-slate-200 bg-white p-3 shadow-sm md:p-4">
                      ...
                      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                        <SearchableSelect
                          ...
                          value={erpOrderForm.paymentMethodCode}
                          options={paymentMethods}
                          loading={loadingErpOrderData}
                          emptyMessage="Não há formas de pagamento sincronizadas. Vá em Configurações > Integração ERP e sincronize Formas de pagamento."
                          onChange={(value) =>
                            setErpOrderField("paymentMethodCode", value)
                          }
                        />
                        <SearchableSelect
                        ...
                        <SearchableSelect
                          ...
                          value={erpOrderForm.priceTableCode}
                          options={priceTables}
                          loading={loadingErpOrderData}
                          emptyMessage="Nenhuma tabela de preço sincronizada. Vá em Configurações > Integração ERP e sincronize Tabelas de preço."
                          onChange={(value) =>
                            setErpOrderField("priceTableCode", value)
                          }
                        />
                        <SearchableSelect
                          label="Filial"
                          value={erpOrderForm.branchCode}
                          options={branches}
                          loading={loadingErpOrderData}
                          emptyMessage="Não há filiais sincronizadas. Vá em Configurações > Integração ERP e sincronize Filiais."
                          onChange={(value) =>
                            setErpOrderField("branchCode", value)
                          }
                        />
                        <div>
                          <SearchableSelect
                        ...
                        <label className="block text-sm">
                          ...
                          <input
                            ...
                            className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-800 shadow-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-100"
                          />
                        </label>
                      </div>
                      ...
                      <div className="border-b border-slate-100 p-3 md:p-4">
                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                          <div>
                            ...
                            <h4 className="text-base font-bold text-slate-900">
                              Itens, estoque disponível e totais
                            </h4>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {itemsWithMissingErp.length ? (
                              <StatusPill tone="danger">
                                Produto sem ERP
                              </StatusPill>
                            ) : (
                        ...
                        <table className="w-full table-fixed divide-y divide-slate-100 text-xs">
                          <thead className="bg-slate-50 text-left text-xs uppercase tracking-wide text-slate-500">
                            <tr>
                              ...
                              <th className="px-2 py-2 font-semibold">Preço</th>
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    <td className="break-words px-2 py-2 text-slate-700">
                                      {opportunityItem.erpProductCode || "-"}
                                    </td>
                                    <td className="break-words px-2 py-2 text-slate-700">
                                      {opportunityItem.product?.className ||
                                        "-"}
                                    ...
                                    <td className="break-words px-2 py-2 font-semibold text-slate-900">
                                      {formatCurrencyBRL(
                                        opportunityItem.netTotal || 0,
                                      )}
                                    </td>
                  ...
                  <aside className="min-w-0 space-y-3 md:space-y-4">
                    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:sticky lg:top-3">
                      ...
                      <div className="mt-4 space-y-2">
                        <div className="flex min-w-0 items-center justify-between gap-2 rounded-xl bg-slate-50 p-2.5 text-xs">
                          <span className="text-slate-500">Valor bruto</span>
                          <strong>
                            {formatCurrencyBRL(
                              opportunityItemTotals.grossTotal ||
                                item.value ||
                                0,
                            )}
                          </strong>
                        ...
                        <div className="flex min-w-0 items-center justify-between gap-2 rounded-xl bg-slate-50 p-2.5 text-xs">
                          ...
                          <strong>
                            {formatCurrencyBRL(
                              opportunityItemTotals.discountTotal || 0,
                            )}
                          </strong>
                        </div>
                        <div className="rounded-xl bg-gradient-to-br from-brand-700 to-slate-900 p-3 text-white shadow-lg shadow-brand-900/20">
                          <span className="text-sm text-brand-100">
                            Total líquido
                          </span>
                          <strong className="mt-1 block break-words text-xl">
                      ...
                      {erpOrderSubmitDisabledReason ? (
                        <div
                          className={`mt-4 rounded-xl border p-3 text-xs ${successfulErpOrder ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-200 bg-amber-50 text-amber-800"}`}
                        >
                          <strong>
                            {successfulErpOrder
                              ? "Pedido ERP já enviado:"
                              : "Pedido ERP indisponível:"}
                          </strong>{" "}
                          {erpOrderSubmitDisabledReason.replace(
                            "Pedido ERP indisponível: ",
                            "",
                          )}
                        </div>
                      ...
                      {erpOrderFeedback ? (
                        <div
                          className={`mt-4 rounded-xl border p-3 text-xs ${erpOrderFeedback.status !== "erro" ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-red-200 bg-red-50 text-red-800"}`}
                        >
                          <p className="font-bold">
                            {erpOrderFeedback.status === "simulado"
                              ? "Simulação ERP validada"
                              : erpOrderFeedback.status === "enviado"
                                ? "Pedido enviado"
                                : "Erro ERP"}
                          </p>
                          {erpOrderFeedback.pedidoIdImportacao ? (
                          ...
                          {erpOrderFeedback.correlationId ? (
                            <p className="mt-1 break-all">
                              correlationId:{" "}
                              <strong>{erpOrderFeedback.correlationId}</strong>
                            </p>
                          ) : null}
                      ...
                      <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50 p-3 text-xs">
                        <div className="mb-2 flex items-center justify-between gap-2">
                          ...
                          <button
                            ...
                            disabled={
                              syncingErpOrderStatus || !erpOrders.length
                            }
                            onClick={onSyncErpOrderStatus}
                          >
                            {syncingErpOrderStatus
                              ? "Atualizando..."
                              : "Atualizar status"}
                          </button>
                        </div>
                        {erpOrders.length ? (
                                ...
                                <div className="mt-2 flex flex-wrap items-end justify-between gap-3">
                                  ...
                                  {order.status === "sent" ? (
                                    <button
                                      ...
                                      disabled={
                                        downloadingErpOrderPdfId === order.id
                                      }
                                      onClick={() =>
                                        onDownloadErpOrderPdf(order)
                                      }
                                    >
                                      {downloadingErpOrderPdfId === order.id
                                        ? "Gerando PDF..."
                          ...
                          <p className="text-xs text-slate-500">
                            Nenhum pedido ERP enviado para esta oportunidade.
                          </p>
                        )}
                      </div>
                      ...
                      >
                        {sendingErpOrder
                          ? erpOrderForm.simulateOnly
                            ...
                            : "Enviando ao ERP..."
                          : erpOrderForm.simulateOnly
                            ? "Validar simulação ERP"
                            : isErpOrderResend
                              ? "Reenviar pedido ao ERP"
                              : "Enviar pedido ao ERP"}
                      </button>
                      <p className="mt-3 text-center text-xs text-slate-500">
                        O backend bloqueia cliente/vendedor sem ERP,
                        oportunidade sem itens, preço zerado, estoque
                        insuficiente e payload inválido antes do envio.
                      </p>
                    </section>
                  </aside>
                </div>
        ...
        >
          ...
          >
            ...
            <div className="mobile-action-stack justify-end">
              ...
              <button
                type="submit"
                className="mobile-primary-button rounded-lg bg-red-600 px-3 py-2 text-sm text-white"
              >
                Confirmar perda
              </button>
            </div>
