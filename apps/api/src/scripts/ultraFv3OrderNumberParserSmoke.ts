...
assert.match(bootstrapSource, /schemaMode === "external"[\s\S]*schema authority is external[\s\S]*schemaMode === "ephemeral-push"[\s\S]*await ensureErpOrderNumberSequence\(\)[\s\S]*validateDatabaseHealth/, "autoridade explícita deve separar produção externa de banco descartável antes de abrir o servidor");
