...
import { hashContact, maskContact } from "./phoneNormalization.js";
...
export class CommunicationIntegrationService {
  constructor(private readonly prisma = defaultPrisma, private readonly providers: CommunicationProvider[] = [new MetaWhatsAppCloudProvider()]) {}
  ...
  async processMetaWhatsAppWebhook(rawBody: Buffer, signature: string | undefined, requestId?: string) {
    ...
    const events = provider.parseEvents(payload); let processed = 0, duplicates = 0;
    for (const event of events) {
      const result = await this.persistEvent(provider, event, payloadHash);
      if (result === "duplicate") duplicates++; else processed++;
    }
    logApiEvent("INFO", "[communications] webhook processed", { requestId, provider: provider.provider, eventCount: events.length, processed, duplicates, durationMs: Date.now() - started });
    return { status: "ok", processed, duplicates };
  }

  private async persistEvent(provider: CommunicationProvider, event: NormalizedCommunicationEvent, payloadHash: string) {
    try {
      return await this.prisma.$transaction(async (tx) => {
        const webhook = await tx.communicationWebhookEvent.create({ data: { channel: provider.channel, provider: provider.provider, externalEventKey: event.externalEventKey, externalAccountId: event.externalAccountId, tenantId: event.tenantId, eventType: event.eventType, payloadHash, status: CommunicationWebhookStatus.PROCESSING, attempts: 1 } }).catch((e) => {
  ...
  async findSafeClientLink(tx: Prisma.TransactionClient, phone?: string) {
    if (!phone) return { reason: "invalid_contact" as const, clientId: null, sellerId: null };
    const digits = phone.replace(/\D/g, "");
    const contacts = await tx.contact.findMany({ where: { OR: [{ phone: { contains: phone } }, { phone: { contains: digits.slice(-8) } }], client: { isArchived: false } }, select: { clientId: true, ownerSellerId: true, client: { select: { id: true, ownerSellerId: true, isArchived: true, code: true } } }, take: 3 });
