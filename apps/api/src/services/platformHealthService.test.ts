...
test("reutiliza métricas persistidas sem duplicar regras de matching", () => { assert.equal(metricFrom({ code_exact: 4 }, "code_exact"), 4); assert.equal(metricFrom(null, "code_exact"), 0); });
test("permissões iniciais são restritas aos perfis operacionais", () => { for (const role of ["diretor", "administrador", "suporte", "ti"]) assert.equal(PLATFORM_HEALTH_ROLES.has(role), true); assert.equal(PLATFORM_HEALTH_ROLES.has("vendedor"), false); });
