...
A correção definitiva mantém no `with.script` apenas o bootstrap constante que clona o SHA esperado e executa `scripts/deploy/preview-remote.sh`. Toda a lógica de configuração, seed, saúde, SHA, ativação e rollback reside nesse arquivo versionado, validado com `bash -n`; não há `bash -c`, `sh -c` ou `eval` aninhado. Na reprodução sanitizada do antigo bloco inline, a linha lógica 25 era a chave `}` que fechava o guard de credencial: o erro apontado ali era efeito do programa concatenado pelo transporte, não um comando funcional dessa linha.
