...
Para validar a ação rápida **Oportunidade**, acioná-la partindo de outra tela e novamente a partir de Oportunidades. Em ambos os casos deve abrir **Nova oportunidade**, sem mensagem de carregamento de identificador, requisição de detalhe ou gravação automática. Cancelar, abrir novamente e usar Voltar; depois abrir normalmente os detalhes de uma oportunidade existente. Repetir como vendedor, gerente e diretor e conferir que Importar, Nova oportunidade, Pipeline e Lista permanecem inteiros e confortáveis para toque.
## Reexecução de ProductPrice após PR827 #30
...
# Retomada do Deploy Production #173 (15/09/2026)

O run `35013697899` bloqueou corretamente antes de parar containers. Para a próxima tentativa, não copie nem renove `/var/log/gest-o/schema/1ad994e31253e100ba69ff7bd50c6c81c55fe19f/applied.tsv`: o consumidor deve validar o bundle original de `20260911190000_product_price_authority` contra seu SHA produtor e então aplicar o predicado compartilhado de equivalência. Somente diferenças nos caminhos exatos `apps/api/prisma/seedPreview.ts` e `apps/api/prisma/validatePreviewTenantReadPilot.ts` são imateriais; schema, migrations ou qualquer terceiro arquivo Prisma bloqueiam tanto `applied.tsv` quanto `tenancy expand roots`.

Checklist de revisão: (1) confirmar checks remotos do novo HEAD; (2) confirmar que o bundle continua regular, sem symlink, `root:600`, dentro do diretório `root:700`; (3) confirmar hash, commit e migration pelo validador existente; (4) confirmar equivalência restrita; (5) manter o diff Prisma ao vivo vazio antes de qualquer parada. Evidência inválida nunca pode chegar à equivalência. A última capacidade informada (`4731516` KiB disponíveis para `5242880` KiB exigidos; déficit `511364` KiB) é pendência separada: não baixe o limite nem exclua backups. Esta documentação não autoriza merge, deploy, schema apply, Recovery, limpeza ou cutover.
# Inventário de imagens e previews legados (15/09/2026)
...
Para legados, construa manualmente um TSV revisado e rode primeiro `bash scripts/legacy-preview-cleanup.sh inventory <manifesto>`. O modo apply é uma operação futura, aprovada e em lote unitário; exige confirmação literal e IDs completos, revalida ownership e aborta se encontrar `gest-o_pgdata`. Ele nunca remove volumes nem executa Compose antigo. Não selecionar por prefixo, não criar labels artificiais e não executar prune amplo. Medir filesystem/Docker e saúde/SHA antes e depois de cada lote; divergência interrompe a janela.
