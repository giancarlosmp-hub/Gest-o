...
# Adendo de implantação do schema — 2026-08-28

A investigação funcional não autoriza resolver o pedido nem executar sync. A migration que sustenta a resolução manual passou a integrar um runner produtivo explicitamente allowlisted, com predecessor/checksum, ledger Prisma atômico, catálogo exato, diff vazio e idempotência. Isso remove apenas o bloqueio de implementação do runner; não muda o pedido real, não executa produção e não torna o deploy seguro antes de merge e checks remotos verdes. Em rollback da aplicação, o schema expandido deve permanecer para preservar auditoria e compatibilidade com a API anterior.
