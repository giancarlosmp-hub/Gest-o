...
A causa do `HARNESS_STEP=post_diff` não era um defeito nos objetos de Pedidos: o harness histórico materializava o predecessor de `20260808120000_tenancy_expand_roots`, aplicava somente essa migration e `20260827190000_add_erp_order_manual_resolution`, mas comparava o banco resultante com o `schema.prisma` corrente. Quando `20260904120000_orders_operational_view` passou a existir, todos os seus objetos posteriores surgiram legitimamente no diff e o gate falhou. A correção preserva a prova histórica e muda apenas seu alvo para o schema do commit introdutor da última migration que o próprio harness aplica; a sequência completa até o schema corrente passa a ter uma prova PostgreSQL 16 própria, sem filtro ou exclusão de objetos de Pedidos.
