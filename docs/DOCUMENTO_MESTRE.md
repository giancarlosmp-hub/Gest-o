...
O comentário de preview apontava para um canal seguro inexistente: email e senha eram derivados deterministicamente do número da PR no workflow. O job agora usa o GitHub Environment `preview` e exige os secrets sintéticos `PREVIEW_AUTH_EMAIL` e `PREVIEW_AUTH_PASSWORD`, sem fallback e sem credenciais produtivas. O proprietário autorizado deve cadastrar/rotacionar os valores em **Settings → Environments → preview → Environment secrets** e entregá-los ao testador somente pelo gerenciador de senhas/canal privado corporativo; o GitHub não permite recuperar o valor depois de salvo.
