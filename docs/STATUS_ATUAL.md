> **Production Schema PR827 — env (run `33196976100`, job `98936493036`).** O preview chegou por SSH ao checkout atualizado de `main` e iniciou o runner, mas parou antes do schema porque o runner pressupunha uma fonte diferente do contrato produtivo real (`legacy_copy`). Nenhuma migration foi aplicada e produção não foi modificada. O desenho corretivo, gates preservados e bloqueio de rerun antes de merge + `main` verde estão na [investigação sanitizada](investigations/production-schema-pr827-env-resolution-2026-08.md). `READY_TO_MERGE_PR827_ENV_FIX=NO` até checks remotos verdes e `READY_TO_RERUN_PREVIEW=NO` até correção mesclada e main verde.
