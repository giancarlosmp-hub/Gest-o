...
## Hotfix — Deploy Production build / par canônico (2026-08-26)

O primeiro Deploy Production após a PR #819, run `33018000891`, job `98341139730`, em `main` e `phase=build`, aprovou o contrato protegido (`ERP_LEGACY_SOURCE_IMMUTABLE=PASS`) e falhou antes do build com `PRODUCTION_PREFLIGHT_FAILURE=backup_path_mismatch`. O call graph era **workflow Deploy Production → `production-deploy-entrypoint.sh` → `deploy-production.sh` → `production-preflight.sh`**: o overlay `legacy_copy` ainda injetava os hints históricos, enquanto somente o ERP Recovery fazia rebind explícito antes do preflight. Não foi falha de operador, `expected_main_sha`, idade, integridade ou ausência. Nenhum build, cutover ou Recovery foi executado e produção não foi modificada.

A resolução agora pertence ao contrato compartilhado `production-backup-common.sh` e é aplicada pelo próprio `production-preflight.sh`: `PRODUCTION_BACKUP_AUTHORIZED_DIRECTORY` (alias compatível `PRODUCTION_BACKUP_AUTHORIZED_DIR`) é a única autoridade e deriva exclusivamente `production.sql.gz` e `production.sql.gz.sha256`. Os valores históricos `PRODUCTION_BACKUP_FILE` e `PRODUCTION_BACKUP_SHA256_FILE` são hints não autoritativos. Preparador, Deploy build e ERP Recovery usam o mesmo resolver; o rebind particular do Recovery foi removido. O validador separado mantém `backup_path_mismatch` para divergência real do par já resolvido, além de manifesto estrito, SHA-256, symlink/escape, TOCTOU, epoch seconds e freshness fail-closed. Nenhum env persistente é alterado por esta correção.

Estado: `ERP_AUTOMATIC_SYNC=NOT_PROVEN`, `INC_ERP_5050=INVESTIGATING`, `READY_FOR_1_0B_2_O=NO` e `READY_TO_MERGE_DEPLOY_CANONICAL_FIX=NO` até checks remotos verdes.
