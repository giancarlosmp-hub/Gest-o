...
| 2. Arquitetura | Visão de componentes, limites e decisões duradouras | 🟡 Parcialmente comprovado | [`DEPLOY_GUIDE.md`](DEPLOY_GUIDE.md#2-arquitetura-ponta-a-ponta); [`TENANCY_ASSESSMENT.md`](TENANCY_ASSESSMENT.md); [`adr/README.md`](adr/README.md) | A Sprint 0.6 inventaria componentes e dependências para tenancy e a ADR 003 aceita define a boundary futura; contratos gerais, ownership completo e diagramas dos demais domínios continuam pendentes. | Acoplamento e mudanças incompatíveis sem análise de impacto. | Executar os gates da ADR 003 e estender o catálogo verificável aos demais domínios. | Alta | Inventário e ADRs vigentes | Arquiteto de Software | Épico 3 |
...
## Atualização controlada — Sprint 1.0A

Somente as dimensões autorizadas são atualizadas abaixo. O desenho não prova execução; Multiempresa
permanece 🔴 e nenhuma outra classificação da matriz é alterada.

| Dimensão | Classificação | Evidência acrescentada | Lacuna remanescente |
|---|---|---|---|
| Arquitetura | 🟡 Parcialmente comprovado | ADR 003 aceita, control plane e contratos no [Brief](sprints/SPRINT_1_0A_MULTI_TENANCY_FOUNDATION.md) | Schema e data access ainda não implementados |
| Segurança | 🔴 Crítico ou não conforme | [Threat model STRIDE](security/MULTI_TENANCY_THREAT_MODEL.md) e provas negativas do scaffolding | Controles não aplicados aos handlers/banco/integrações |
