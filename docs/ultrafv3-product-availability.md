...
## Diagnóstico e Comprovação da Tabela 2 (Setembro/2026)

Fatos comprovados sobre a Tabela 2 (`2 · CONSUMIDOR FINAL`) e fluxo de seleção/sincronização:

1. **Ausência de linhas `ProductPrice` para Tabela 2**: Nenhuma linha `ProductPrice` é criada para a tabela `2` nos fluxos atuais de sincronização automática ou manual.
2. **Persistência restrita de `/priceVariations`**: A rota `/priceVariations` do UltraFV3 é persistida exclusivamente em `AppConfig` (chave `erp.ultrafv3.priceVariations`) e nunca cria/atualiza registros na tabela `ProductPrice`.
3. **Decisão de design em `calculateOpportunityPriceForTable`**: A função em `apps/api/src/services/opportunityPriceService.ts` (linhas ~180–225) define `const rawTablePrice = null;`, desativando deliberadamente qualquer fallback dinâmico em tempo de seleção. O filtro de busca em `POST /products/search` exige estritamente um registro `ProductPrice > 0` para a tabela selecionada; ausência do registro resulta em zero resultados visíveis.
4. **Identidade de ordem entre os botões**: Tanto o botão **Atualizar estoque** (na oportunidade) quanto a **Sincronização Completa ERP** (em Configurações) executam a mesma ordem autoritativa (`syncProducts` → `syncPrices`). A divergência percebida pelo usuário é de resultado (tabela 2 sem resultados em ambos os fluxos) e não de caminho/ordem de execução de código.
5. **Escopo amplo de tabelas**: Este comportamento afeta a Tabela 2 e qualquer outra tabela além da Tabela 1 (Tabela 3, 4, etc.) que dependa de `/priceVariations` ou que não venha como preço explícito no snapshot de `/prices`.
