...
export const parseCatalogRows = content => content.trim().split("\n").filter(Boolean).map(line => {
  const columns = line.split("\t");
  if (columns.length !== 4) throw new Error(`CATALOG_TSV_FIELD_COUNT_MISMATCH:${columns[1] || "unknown"}:${columns.length}`);
  const [kind, object, position, detail] = columns;
  return { kind, object, position: Number(position), detail };
});

export function validateCatalogFile(input) {
  const rows = parseCatalogRows(readFileSync(input, "utf8"));
  const expectedNames = {
