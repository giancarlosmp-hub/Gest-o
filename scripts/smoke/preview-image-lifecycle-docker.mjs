...
writeFileSync(modeFile, 'ambiguous_multiple_prs');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /run_pr_correlation_missing/);
assert.ok(failureImages.every(image => spawnSync('docker', ['image', 'inspect', image.image_id]).status === 0), 'multiple PRs on same branch/SHA preserves all images');
writeFileSync(modeFile, 'explicit_mismatched_pr');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /run_pr_correlation_missing/);
assert.ok(failureImages.every(image => spawnSync('docker', ['image', 'inspect', image.image_id]).status === 0), 'explicit mismatched PR preserves all images');
writeFileSync(modeFile, 'paginated_ambiguous_page2');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /run_pr_correlation_missing/);
assert.ok(failureImages.every(image => spawnSync('docker', ['image', 'inspect', image.image_id]).status === 0), 'page 2 ambiguity preserves all images');
writeFileSync(modeFile, 'branch_reuse_different_shas');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /run_pr_correlation_missing/);
assert.ok(failureImages.every(image => spawnSync('docker', ['image', 'inspect', image.image_id]).status === 0), 'branch reuse across PRs preserves all images');
writeFileSync(modeFile, 'api_incomplete_response');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /run_pr_correlation_missing/);
assert.ok(failureImages.every(image => spawnSync('docker', ['image', 'inspect', image.image_id]).status === 0), 'incomplete API response preserves all images');
writeFileSync(modeFile, 'explicit_contradiction_top_42_attempt_99');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /run_pr_correlation_missing/);
assert.ok(failureImages.every(image => spawnSync('docker', ['image', 'inspect', image.image_id]).status === 0), 'top 42 vs attempt 99 contradiction preserves all images');
writeFileSync(modeFile, 'explicit_contradiction_top_99_attempt_42');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /run_pr_correlation_missing/);
assert.ok(failureImages.every(image => spawnSync('docker', ['image', 'inspect', image.image_id]).status === 0), 'top 99 vs attempt 42 contradiction preserves all images');
writeFileSync(modeFile, 'explicit_sha_mismatch_top_divergent');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /authenticated_run_identity_diverged field=run_pull_request_head_sha/);
assert.ok(failureImages.every(image => spawnSync('docker', ['image', 'inspect', image.image_id]).status === 0), 'explicit top SHA mismatch preserves all images');
writeFileSync(modeFile, 'explicit_sha_mismatch_attempt_divergent');
result = run(process.execPath, [join(root, 'scripts/preview-image-lifecycle.mjs'), 'cleanup', failureManifest], { env: identity });
assert.notEqual(result.status, 0); assert.match(result.stderr, /authenticated_run_identity_diverged field=run_pull_request_head_sha/);
