...
for (const marker of ["TENANT_READ_PREVIEW_SEED=PASS", "TENANT_READ_PREVIEW_DATASET=PASS", "TENANT_READ_PREVIEW_SHADOW=MATCH", "TENANT_READ_PREVIEW_HTTP=200,200,200,200", "DEFAULT_TENANT_ID=tenant-default-v1", "TENANT_READ_SHADOW_EVENT="]) assert.ok(preview.includes(marker), `missing ${marker}`);
...
assert.match(preview, /trap rollback_and_cleanup EXIT[\s\S]*PILOT_ACTIVATED=true/, "rollback must cover every failure after activation");
