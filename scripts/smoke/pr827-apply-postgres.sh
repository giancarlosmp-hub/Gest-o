...
assert_clean_worktree(){ local phase=$1 status line; status=$(git status --porcelain=v1 --untracked-files=all); if [[ -n $status ]]; then printf 'HARNESS_WORKTREE_%s=FAIL\n' "$phase" >&3; while IFS= read -r line; do printf 'HARNESS_DIRTY_PATH_CLASS=%s PATH=%s\n' "${line:0:2}" "${line:3}" >&3; done <<<"$status"; return 1; fi; printf 'HARNESS_WORKTREE_%s=PASS\n' "$phase" >&3; }
