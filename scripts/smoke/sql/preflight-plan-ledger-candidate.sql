...
DECLARE existing public.tenant_preflight_evidence_registry; inserted boolean := false;
BEGIN
 ...
 ON CONFLICT (evidence_id) DO NOTHING RETURNING * INTO existing;
 inserted := FOUND;
 IF NOT inserted THEN
   SELECT * INTO existing FROM public.tenant_preflight_evidence_registry WHERE evidence_id=p_id FOR UPDATE;
   IF existing.evidence_hash<>p_hash OR existing.contract_version<>p_contract OR existing.inventory_version<>p_inventory OR existing.generated_at<>p_generated OR existing.expires_at<>p_expires OR existing.result<>p_result THEN
     RAISE EXCEPTION USING ERRCODE='23505', MESSAGE='conflicting evidence replay';
   END IF;
 END IF;
 INSERT INTO public.tenant_backfill_plan_event(event_type,evidence_id) VALUES(CASE WHEN inserted THEN 'EVIDENCE_REGISTERED' ELSE 'IDEMPOTENT_REPLAY' END,p_id);
 RETURN CASE WHEN inserted THEN 'REGISTERED' ELSE 'IDEMPOTENT_REPLAY' END;
END $$;
...
RETURNS text LANGUAGE plpgsql SECURITY DEFINER SET search_path=pg_catalog,public AS $$
DECLARE e public.tenant_preflight_evidence_registry; existing public.tenant_backfill_plan_ledger; inserted boolean := false;
BEGIN
 ...
 IF e.result<>'READY' OR e.expires_at<=statement_timestamp() THEN RAISE EXCEPTION USING ERRCODE='23514', MESSAGE='evidence is not eligible'; END IF;
 IF p_dry IS DISTINCT FROM true OR p_apply IS DISTINCT FROM false OR p_status<>'PLANNED' THEN RAISE EXCEPTION USING ERRCODE='23514', MESSAGE='unsafe plan'; END IF;
 ...
 ON CONFLICT (plan_id) DO NOTHING RETURNING * INTO existing;
 inserted := FOUND;
 IF NOT inserted THEN
  SELECT * INTO existing FROM public.tenant_backfill_plan_ledger WHERE plan_id=p_id FOR UPDATE;
  IF existing.plan_hash<>p_hash OR existing.evidence_id<>p_evidence_id OR existing.evidence_hash<>p_evidence_hash OR existing.plan_version<>p_version OR existing.target_tenant_id<>p_tenant THEN
   RAISE EXCEPTION USING ERRCODE='23505', MESSAGE='conflicting plan replay'; END IF;
 END IF;
 INSERT INTO public.tenant_backfill_plan_event(event_type,evidence_id,plan_id) VALUES(CASE WHEN inserted THEN 'PLAN_REGISTERED' ELSE 'IDEMPOTENT_REPLAY' END,p_evidence_id,p_id);
 RETURN CASE WHEN inserted THEN 'REGISTERED' ELSE 'IDEMPOTENT_REPLAY' END;
