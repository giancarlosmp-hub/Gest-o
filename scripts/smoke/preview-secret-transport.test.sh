#!/usr/bin/env bash
set -euo pipefail
sentinel=$(mktemp -u); log=$(mktemp); trap 'rm -f "$sentinel" "$log"' EXIT
...
PREVIEW_SEED_PASSWORD="$decoded" node -e 'const c=require("crypto");const actual=c.createHash("sha256").update(process.env.PREVIEW_SEED_PASSWORD).digest("hex");if(actual!==process.argv[1])process.exit(1)' "$expected_hash" >"$log"
test ! -e "$sentinel"
! grep -Fq "$secret" "$log"
